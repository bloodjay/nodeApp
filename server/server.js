...
io.on('connection',(socket) => {
    ...
    socket.on('createMessage', (message, callback) => {
        ...
        {
        io.to(user.room).emit('newMessage',generateMes(user.name,message.text))
        }
        callback();
    ...
    socket.on('createLocationMessage',(coords)=>{
        var user = users.getUser(socket.id);
        if(user)
        io.to(user.room).emit('newLocationMessage',generateLocationMes(user.name,coords.latitude,coords.longitude))
    });
